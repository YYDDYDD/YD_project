Shader "Custom/charactor_skin_shader" {
Properties{
		// --- Albedo ---
		...
		// 白黒のマスクテクスチャで部位ごとに調整（白=アニメ風のフラットな影 / 黒=通常のグラデーション影）
		[Toggle(_TOON_SHADING_ON)] _UseToonShading("Use Toon Shadow (Anime Style)", Float) = 0
		_ToonShadowTex("Toon Shadow Mask (White=Toon / Black=Realistic)", 2D) = "white" {}
		...
		_ToonShadowInfluence("Toon Shadow Influence", Range(0,1)) = 1.0
	}
	...
	SubShader{
		...
		struct SurfaceOutputCharacter {
			...
			half ToonShadow;         // 白黒マスク由来: アニメ風トゥーン影の影響度 (0=通常の陰影 / 1=フラットなアニメ影)
		};

		// --- カスタムライティング関数 ---
		// Diffuse(拡散反射) / Specular(鏡面反射, Blinn-Phong近似) を自前で計算することで、
		// 以下の4点をテクスチャで部位ごとに独立してコントロールできるようにしている。
		//   ・R: ライトの減衰(atten)の影響
		//   ・G: 自身のNdotLに起因する陰の影響
		//   ・B: 環境(ステージ)の反射の影響
		//   ・トゥーン(アニメ風)の階調のない陰への切り替え度合い（白黒マスクで制御）
		...
		inline half4 LightingCharacterSpecular(SurfaceOutputCharacter s, half3 viewDir, UnityGI gi)
		{
			...
			// --- アニメ風トゥーンシャドウ（影の影響を受けにくくするパラメーター、白黒マスクで部位ごとに制御） ---
			#if defined(_TOON_SHADING_ON)
				half toonAmount = saturate(s.ToonShadow * _ToonShadowInfluence);
				half toonMask = smoothstep(_ToonShadowThreshold - _ToonShadowSoftness, _ToonShadowThreshold + _ToonShadowSoftness, ndotlDiffuse);
				ndotlDiffuse = lerp(ndotlDiffuse, toonMask, toonAmount);
			#endif
		...
		void surf(Input IN, inout SurfaceOutputCharacter o) {
			...
			// --- Toon Shadow Mask (白黒: 白=アニメ風の影 / 黒=通常の影) ---
			o.ToonShadow = tex2D(_ToonShadowTex, IN.uv_MainTex).r;
		}
		ENDCG
	}
